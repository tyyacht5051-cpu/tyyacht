...
const migrations: Migration[] = [
  ...
  },
  {
    ...
    up: () => {
      try { db.exec(`ALTER TABLE notices ADD COLUMN published BOOLEAN DEFAULT 1`); } catch (e: any) {
        if (!e.message.includes('duplicate column name')) throw e;
      }
    },
  },
  ...
  },
  {
    ...
    up: () => {
      try { db.exec(`ALTER TABLE exemption_schedules ADD COLUMN is_closed BOOLEAN DEFAULT 0`); } catch (e: any) {
        if (!e.message.includes('duplicate column name')) throw e;
      }
    },
  },
...
export function runMigrations() {
  ...
  const applied = db.prepare('SELECT id FROM schema_migrations').all() as { id: number }[];
